#!/usr/bin/env python3
"""
Deploy Updated Companies Analytics Function
Apply the corrected SQL function to Supabase
"""

from supabase_utils import get_client

# Updated function with corrected market logic
UPDATED_COMPANIES_FUNCTION = """
...
def deploy_updated_function():
    """Deploy the updated companies analytics function"""
    try:
        client = get_client()
        if not client:
        ...
        print("🔄 Deploying updated companies analytics function...")

        # Execute the updated function definition
        result = client.rpc('sql', {'query': UPDATED_COMPANIES_FUNCTION}).execute()

        print("✅ Updated function deployed successfully!")
        print("🔄 Testing the updated function...")

        # Test the function
        test_result = client.rpc('refresh_companies_analytics', {'days_back': 60}).execute()

        if test_result.data:
            response = test_result.data
            print(f"✅ Test completed: {response}")
            ...
            print("❌ No response from test function")
            return False
...
if __name__ == "__main__":
    ...
    success = deploy_updated_function()

    if success:
        print("\n✅ Function deployment and test completed successfully!")
        print("🌐 The manual refresh button should now work correctly")
    else:
