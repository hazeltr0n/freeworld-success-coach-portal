  ...
  try {
    ...
    if (!candidate_id || !job_url) {
      return new Response(JSON.stringify({
        success: false,
        message: 'Missing required fields: candidate_id and job_url'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data, error } = await supabase.from('job_feedback').insert({
      candidate_id,
      job_id: job_id || null,
      job_url,
    ...
    const permanentNegativeTypes = [
      'requires_experience',
      'not_fair_chance_friendly'
    ];
    const temporaryNegativeTypes = [
      'job_expired'
    ];
    const positiveTypes = [
      'i_like_this_job',
      'i_applied_to_this_job'
    ];
    ...
    if (permanentNegativeTypes.includes(feedback_type)) {
      ...
      if (job_id) {
        try {
          const { data: jobData, error: fetchError } = await supabase.from('jobs').select('feedback_expired_links').eq('job_id', job_id).single();
          ...
          if (!fetchError && jobData) {
            ...
            if (!updateError) {
              ...
              console.log(`Job ID update failed: ${updateError.message}`);
            }
          } else {
            console.log(`Job ID lookup failed: ${fetchError?.message}`);
          }
        } catch (error) {
          console.log(`Job ID lookup failed: ${error.message}`);
        }
      } else {
      ...
      if (job_id) {
        try {
          const { data: jobData, error: fetchError } = await supabase.from('jobs').select('feedback_likes').eq('job_id', job_id).single();
          ...
          if (!fetchError && jobData) {
            ...
            if (!updateError) {
              ...
              console.log(`Job ID likes update failed: ${updateError.message}`);
            }
          } else {
            console.log(`Job ID likes lookup failed: ${fetchError?.message}`);
          }
        } catch (error) {
          console.log(`Job ID likes lookup failed: ${error.message}`);
        }
      } else {
      ...
      if (feedback_type === 'i_applied_to_this_job' && candidate_id) {
        ...
        try {
          const { data: agentData, error: fetchError } = await supabase.from('agent_profiles').select('total_applications').eq('agent_uuid', candidate_id).single();

          if (!fetchError && agentData) {
            const currentApps = agentData.total_applications || 0;
            const { error: agentError } = await supabase.from('agent_profiles').update({
            ...
            if (agentError) {
              ...
              // Trigger analytics refresh for all agents
              try {
                const { error: refreshError } = await supabase.rpc('refresh_free_agents_analytics');

                if (refreshError) {
                  console.error('Error refreshing agent analytics:', refreshError);
                } else {
                  console.log('Successfully refreshed agent analytics');
                }
              } catch (refreshErr) {
                console.error('Exception during analytics refresh:', refreshErr);
              }
            }
          } else {
