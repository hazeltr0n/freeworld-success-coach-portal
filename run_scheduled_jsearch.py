...
def main():
    ...
    try:
        ...
        if notification_email:
            ...
            error_details = {
                ...
                'storage_error': str(e),
                'markets_searched': list(markets.keys()) if 'markets' in dir() else [],
                'run_id': run_id if 'run_id' in dir() else 'unknown'
            }
            try:
                send_zero_jobs_alert("JSearch Scheduled", error_details, notification_email)
    ...
    if notification_email and upload_count == 0:
        ...
        error_details = {
            ...
            'after_dedup': total_classified_jobs,
            'classification_errors': 0,
            'storage_error': '',
            'markets_searched': list(markets.keys()),
            'run_id': run_id
        }
