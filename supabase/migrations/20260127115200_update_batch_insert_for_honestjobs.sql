-- Update batch_insert_jobs_with_dedup to include HonestJobs fields
...
BEGIN
    ...
    SELECT
        ...
        -- HonestJobs specific fields
        (job_record->>'honestjobs_id')::TEXT,
        COALESCE((job_record->>'verified_hires')::INTEGER, 0),
        COALESCE((job_record->>'is_fair_chance')::BOOLEAN, FALSE),
        (job_record->>'employer_id')::TEXT
    FROM jsonb_array_elements(p_jobs_data) as job_record
    ON CONFLICT (job_id) DO UPDATE SET
        -- Update ALL fields when same job_id is re-uploaded (may have new R3 hash)
        ...
        -- HonestJobs specific fields
        honestjobs_id = EXCLUDED.honestjobs_id,
        verified_hires = EXCLUDED.verified_hires,
        is_fair_chance = EXCLUDED.is_fair_chance,
    ...
    -- This runs AFTER the insert to clean up similar jobs from different sources
    DELETE FROM jobs a
    USING jobs b
    WHERE a.rules_duplicate_r3 = b.rules_duplicate_r3
      AND a.rules_duplicate_r3 IS NOT NULL
      AND a.updated_at < b.updated_at
      AND a.job_id != b.job_id;  -- Don't delete the same job
...
COMMENT ON FUNCTION batch_insert_jobs_with_dedup IS 'Dual-layer dedup with HonestJobs fair chance fields support';
