-- Add apply_instructions to batch_insert_jobs_with_dedup function
-- For Inside Track jobs without a URL (phone/address to apply)
...
BEGIN
    ...
    SELECT
        ...
        (job_record->>'apply_instructions')::TEXT
    ...
    ON CONFLICT (job_id) DO UPDATE SET
        ...
        apply_instructions = COALESCE(EXCLUDED.apply_instructions, jobs.apply_instructions);
...
COMMENT ON FUNCTION batch_insert_jobs_with_dedup IS 'Dual-layer dedup with Inside Track support: job_id conflict handling + R3 cleanup + apply_instructions';
